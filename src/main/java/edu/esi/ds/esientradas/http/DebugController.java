 ...
 * 
 * long butacas = todas.stream().filter(e ->
 * e.getClass().getSimpleName().equals("Precisa")).count();
 * long zonas = todas.stream().filter(e ->
 * e.getClass().getSimpleName().equals("DeZona")).count();
 * 
 * return Map.of(
 * "total_entradas", todas.size(),
 * "butacas", butacas,
 * "zonas", zonas,
 * "tipos_en_bd", tipos != null ? tipos : List.of("(vacio)"),
