...
public interface EntradaDao extends JpaRepository<Entrada, Long> {
    ...
    @Query("""
        SELECT new edu.esi.ds.esientradas.dto.EstadisticasEspectaculoDTO(
            COUNT(e),
            SUM(CASE WHEN e.estado = 'DISPONIBLE' THEN 1L ELSE 0L END),
            SUM(CASE WHEN e.estado = 'VENDIDA' THEN 1L ELSE 0L END),
            SUM(CASE WHEN e.estado = 'RESERVADA' THEN 1L ELSE 0L END)
        )
        FROM Entrada e
